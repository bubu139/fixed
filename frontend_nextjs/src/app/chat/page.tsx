...
// import { Textarea } from '@/components/ui/textarea'; // <-- Bỏ Textarea cũ
import { MathInput } from '@/components/ui/math-input'; // <-- Import MathInput mới
import { MathfieldElement } from 'mathlive'; // <-- Import Type của MathLive
...
// --- DANH SÁCH CÔNG CỤ "SMART" (CÓ PLACEHOLDER #?) ---
const SMART_TOOLS = [
  { label: 'x²', latex: 'x^2' },                  
  { label: 'aⁿ', latex: '#?^{#?}' },              
  { label: '√',  latex: '\\sqrt{#?}' },           
...
// --- DANH SÁCH KÝ TỰ CHO POPOVER (SIGMA) ---
// Đã cập nhật insert để tương thích tốt hơn
const latexSymbols = [
  {
    label: "Toán tử",
    symbols: [
...
export default function ChatPage() {
  ...
  const [input, setInput] = useState(''); // Đây là chuỗi LaTeX
  ...
  // --- HÀM XỬ LÝ CHÈN KÝ TỰ THÔNG MINH ---
  const handleInsertSymbol = (latex: string) => {
    const mf = mathFieldRef.current;
    if (mf) {
      // Dùng lệnh insert của MathLive để xử lý placeholder (#?)
...
const handleSend = async (e?: FormEvent) => {
    ...
    const apiMessage = normalizedInput || 'Học sinh gửi ảnh/file. Hãy phân tích.';
    ...
    try {
        ...
        // --- Xử lý Mindmap (Chỉ sửa khai báo normalized) ---
  ...
  // Adjust Padding bottom based on input height
  useEffect(() => {
    const adjustPadding = () => {
    ...
    <div className="flex flex-col h-full bg-white rounded-3xl shadow-2xl overflow-hidden border border-blue-100 relative">
      ...
      <div className="flex-1 overflow-y-auto bg-gradient-to-b from-white to-blue-50" ref={scrollAreaRef}>
        <div className="p-6 flex flex-col gap-6">
            ...
            <div key={index} className={cn("flex items-start gap-3", message.isUser ? "justify-end" : "justify-start")}>
              ...
              >
                <div className={cn("prose prose-sm max-w-none break-words", message.isUser ? "prose-invert" : "")}>
                  <ReactMarkdown
                    components={{
                      // Tùy chỉnh hiển thị Markdown nếu cần
          ...
          {/* GEOGEBRA & MINDMAP CARDS (GIỮ NGUYÊN) */}
          {geogebraSuggestion && (
      ...
      <div ref={inputContainerRef} className="fixed bottom-0 left-0 right-0 p-3 sm:px-4 sm:py-4 bg-white border-t border-blue-100 z-10 shadow-[0_-5px_15px_rgba(0,0,0,0.05)]">
        ...
        {/* THANH CÔNG CỤ NHANH (SMART TOOLS) */}
                    ...
                    <div className="grid grid-cols-5 gap-1">
                      {group.symbols.map((symbol) => (
                        <Button
                          ...
                          onClick={() => handleInsertSymbol(symbol.insert)} // Dùng chung hàm insert thông minh
                        >
                          {symbol.display}
                        </Button>
                      ))}
