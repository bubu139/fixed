...
-- Vector search helpers for RAG (chat documents)
...
-- Vector search helpers for RAG (test materials)
create or replace function public.match_test_materials(
  query_embedding extensions.vector,
  match_threshold double precision default 0.0,
  match_count integer default 5,
  p_user_id uuid default null
...
begin
  ...
  select
    ...
    c.source_path,
    c.user_id
  from test_material_chunks c
  join test_materials m on m.id = c.material_id
  where c.embedding is not null
    and (p_user_id is null or c.user_id = p_user_id)
    and (1 - (c.embedding <=> query_embedding)) >= match_threshold
  order by c.embedding <=> query_embedding
